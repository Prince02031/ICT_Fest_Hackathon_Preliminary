...
* **Location:** `app/routers/bookings.py` (line 148)
...
* **Location:** `app/routers/bookings.py` (line 149)
...
* **Location:** `app/routers/bookings.py` (line 147)
...
* **Location:** `app/serializers.py` (lines 12–16)
...
* **Location:** `app/routers/auth.py` (lines 34–40)
...
* **Location:** `app/routers/bookings.py` (lines 217–218)
...
* **Location:** `app/routers/bookings.py` (line 213)
...
* **Location:** `app/routers/bookings.py` (lines 92–93)
...
* **Location:** `app/routers/bookings.py` (lines 95–100)
...
* **Location:** `app/timeutils.py` (lines 17–18)
...
* **Location:** `app/auth.py` (lines 52, 59)
...
* **Location:** `app/auth.py` (lines 89, 116)
...
* **Location:** `app/routers/bookings.py` (lines 174–175)
...
* **Location:** `app/routers/auth.py` (line 83) & `app/auth.py` (lines 96–105)
...
* **Location:** `app/services/export.py` (lines 32–38)
...
* **Location:** `app/services/refunds.py` (line 15)
...
* **Location:** `app/services/notifications.py` (lines 24–35)
...
* **Location:** `app/routers/bookings.py` (lines 194–238)
...
## 20. Input Validation for Malformed DateTime Inputs
* **Location:** `app/timeutils.py` (lines 13–16)
* **Bug:** When creating a booking with a malformed datetime string (e.g. non-ISO 8601), the application failed to validate the format in Pydantic (which takes `str` directly to bypass default parsing) and threw an unhandled Python `ValueError`, crashing with a `500 Internal Server Error`.
* **Fix:** Wrapped the ISO 8601 datetime parsing logic in a try-except block, raising the correct `400 INVALID_BOOKING_WINDOW` error response upon receiving an invalid format.
